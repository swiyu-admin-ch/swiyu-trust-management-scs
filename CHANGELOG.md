...
- fix transaction issue for auto-renewal of trust statements
- update from status list update v1 to v2

## 3.37.1

### Changed

- Auto-Renewal: Fix transaction issue for auto-renewal of trust statements

## 3.37.0

### Changed

- Refactor AddDidSubmission - Approval to issue BPI and not statements directly
- Update jeap-spring-boot-parent -> 41.5.0
...
- Auto-Renewal: Add trust statement refresh cronjobs
...
- Add Protected Issuance Authorization service
- Protected Issuance Authorization Demo data

### Changed

- Refactor Protected Issuance Entry data model
- Internal handling of BusinessPartnerIdentityStatus decoupled from event message type
- Event handlers do use their own service id now

## 3.34.5

### Added

- Add Protected Verification Authorization service

### Changed

- Rework and align demo data

## 3.34.4

### Added

- Add demo data for BusinessPartnerIdentity

## 3.34.3

### Changed

- Refactor trust statement localized fields to use plain locale strings instead of `Language` enums

## 3.34.2

### Added

- Add Kafka consumer class and configuration for Business Partner Identity Events
...
- Force Hibernate's JSON column mapper to Jackson 3 (`hibernate.type.json_format_mapper: jackson3`), since it otherwise
  defaults to Jackson 2 whenever Jackson 2 is present on the classpath at all
- Add a new Flyway migration for the `deferred_message.sampled` column added by the updated jeap-messaging-outbox
- Add `spring-boot-security-test` test dependency, required since Spring Boot 4 moved MockMvc's Spring Security test
  wiring into its own module
...
- Trust onboarding task resubmission workflow
- Overdue filter and highlighting for trust onboarding tasks
- Task type filter for the trust onboarding task list

### Changed

- Trust onboarding task list filters now apply automatically and were reordered (submission date, status, task type,
  assignee)
