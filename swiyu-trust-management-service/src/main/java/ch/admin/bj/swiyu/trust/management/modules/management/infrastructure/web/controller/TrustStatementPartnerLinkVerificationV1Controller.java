...
import ch.admin.bj.swiyu.trust.management.modules.management.api.TrustStatementPartnerLinkVerificationV1RequestDto;
...
@Slf4j
@AllArgsConstructor
@Tag(name = "TrustStatementPartnerLink Verification")
@RestController
...
public class TrustStatementPartnerLinkVerificationV1Controller {
    ...
    @Operation(
        summary = "IF-012.131 - Prepare a new trust statement request. If confirmed it gets also issued and published."
    )
    public ResponseEntity<TrustStatementPartnerLinkDto> issueAndPublishVerificationTrustStatement(
        ...
        @Valid @RequestBody TrustStatementPartnerLinkVerificationV1RequestDto request
    ) {
        var statement = this.trustStatementService.issueAndPublishVerificationTrustStatement(partnerId, request);
