...
public class TrustStatementPartnerLinkIdentityV1Controller {
    ...
    @Operation(
        summary = "IF-012.111 - Prepare a new trust statement request. If confirmed it gets also issued and published."
    )
    public ResponseEntity<TrustStatementPartnerLinkDto> issueAndPublishIdentityTrustStatement(
        ...
        @Valid @RequestBody TrustStatementPartnerLinkIdentityV1RequestDto request
    ) {
        var statement = this.trustStatementService.issueAndPublishIdentityTrustStatement(partnerId, request);
