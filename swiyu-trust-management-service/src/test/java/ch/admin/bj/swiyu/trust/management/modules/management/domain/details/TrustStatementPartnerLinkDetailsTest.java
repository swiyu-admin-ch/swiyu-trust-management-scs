...
class TrustStatementPartnerLinkDetailsTest {
    ...
    void verifyAllTypesAreConfiguredForJsonSerialization() {
        ...
        for (TrustStatementPartnerLinkType type : TrustStatementPartnerLinkType.values()) {
            assertTrue(
        ...
        for (String configuredSubTypeName : configuredSubTypeNames) {
            assertTrue(
                Arrays.stream(TrustStatementPartnerLinkType.values()).anyMatch(type ->
                    type.name().equals(configuredSubTypeName)
                ),
