...
import ch.admin.bj.swiyu.trust.management.modules.management.api.TrustStatementPartnerLinkIssuanceV1RequestDto;
...
@RestController
@RequestMapping("/api/v1/trust-statement-partner-links/issuance")
public class TrustStatementPartnerLinkIssuanceV1Controller {
    ...
    @Operation(
        summary = "IF-013.121 - Prepare a new trust statement request. If confirmed it gets also issued and published."
    )
    public ResponseEntity<TrustStatementPartnerLinkDto> issueAndPublishIssuanceTrustStatement(
        @RequestParam(required = false) @Parameter(
            description = "UUID of the partner. Currently still optional since difficult to know when onboarding trust statements."
        ) UUID partnerId,
        @Valid @RequestBody TrustStatementPartnerLinkIssuanceV1RequestDto request
    ) {
        var statement = this.trustStatementService.issueAndPublishIssuanceTrustStatement(partnerId, request);
